...
run_order() {
  ...
  docker run --rm "mcr.microsoft.com/dotnet/aspnet:${RUNTIME_VERSION}" \
  ...
  curl -sS -D "$case_dir/05-bob-nocache.headers" \
    -o "$case_dir/05-bob-nocache.body" \
    -b "$case_dir/bob.cookies" \
    "http://127.0.0.1:${port}/private-nocache"

  curl -sS -D "$case_dir/06-anonymous-private.headers" \
    -o "$case_dir/06-anonymous-private.body" \
    -w '%{http_code}\n' \
    "http://127.0.0.1:${port}/private" \
    > "$case_dir/06-anonymous-private.status"
  ...
  if [[ "$expectation" == vulnerable ]]; then
    ...
    grep -F 'PRIVATE_USER=alice;ACCOUNT=account-alice;EXEC_COUNT=1;PRIMARY_AUTH=False;ANY_AUTH=True;AUTH_METADATA=True;ALLOW_ANON=False;IDENTITY_COUNT=2' \
    ...
    grep -F 'PRIVATE_USER=alice;ACCOUNT=account-alice;EXEC_COUNT=1;PRIMARY_AUTH=True;ANY_AUTH=True;AUTH_METADATA=True;ALLOW_ANON=False;IDENTITY_COUNT=2' \
      "$case_dir/02-alice-private.body" >/dev/null
    grep -F 'PRIVATE_USER=bob;ACCOUNT=account-bob;EXEC_COUNT=2;PRIMARY_AUTH=True;ANY_AUTH=True;AUTH_METADATA=True;ALLOW_ANON=False;IDENTITY_COUNT=2' \
      "$case_dir/04-bob-private.body" >/dev/null
